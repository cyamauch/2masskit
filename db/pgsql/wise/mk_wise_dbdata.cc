 ...
 *
 *  How to use:
 *    $ ./mk_wise_dbdata /somewhere/WISE/wise-allsky
 *    $ ./mk_wise_dbdata http://irsadist.ipac.caltech.edu/wise-allsky
 ...
 */
/* ObiID */
#define OBJID_OFFSET 1000000000LL
/* Number of files for main table */
#define N_MAIN_FILES 460		/* set to < 2 GByte in each file */
/* Number of child tables */
#define N_EQI_CHILD_TABLES 920		/* Should be tuned */
...
#define CAT_NAME "Wise"
#define CAT_LIST_FILE "wise_list.txt"
#define N_CAT_COLS 285
...
#define N_ZONE_ENTRIES 524288
/* Set number of rows in this catalog */
#define N_ALL_ENTRIES 563921584LL
			...
			int *ret_max_deci, bool finish )
{
    ...
    if ( fh_out.openf("w", "wise-allsky-cat-zone%04d.txt.gz", zone_id) < 0 ) {
	...
	for ( l = 0 ; l < N_CAT_COLS ; l++ ) {
	    ...
	    if ( l != 58 && l != 59 && l != 60 && l != 61 && /* w?frtr */
		 l != 281 && l != 282 && l != 283 /* x,y,z */ ) {
	        /* check null */
	        if ( width == 0 ) {
...
int main( int argc, char *argv[] )
{
	...
	sio.eprintf("[Example]\n");
	sio.eprintf("  $ %s /somewhere/WISE/wise-allsky\n",argv[0]);
	sio.eprintf("  $ %s http://irsadist.ipac.caltech.edu/wise-allsky\n", argv[0]);
	goto quit;
    }
    dir_name = argv[1];
	...
	for ( i=0 ; i < cat_file_list.length() ; i++ ) {
	    ...
	    tstring col_top(48);		/* designation, ra, dec only */
	    const char *l_ptr;
		...
		if ( zflg[zone_id] < 0 ) {
		    ...
		    if ( zout[zone_id].openf("wb1", /* cmp-level-1 => fast */
					     "_wise-allsky-cat-zone%04d.txt.gz",
					     zone_id) < 0 ) {
	...
	filename.printf("_wise-allsky-cat-zone%04d.txt.gz",(int)i);
	...
	while ( (l_ptr=cat_in.getline()) != NULL ) {
		...
		/* obtain dai, deci, cx, cy, cz */
		rai = RA2RAI(col_top.atof(p0));
		deci = DEC2DECI(col_top.atof(p1));
		if ( DEC2DECI(90.0) < deci ) deci = DEC2DECI(90.0);
		else if ( deci < DEC2DECI(-90.0) ) deci = DEC2DECI(-90.0);
