...
CREATE FUNCTION fTycho2GetNearestObjIDEq(FLOAT8,FLOAT8,FLOAT8)
  RETURNS INT4
...
CREATE FUNCTION fTycho2GetNearbyObjEq(FLOAT8,FLOAT8,FLOAT8)
...
CREATE FUNCTION fTycho2GetNearbyObjCel(TEXT, FLOAT8, FLOAT8, FLOAT8)
  RETURNS SETOF tObjCelAndDistance
  AS 'SELECT o.*
      FROM (
       SELECT objid, 
...
CREATE FUNCTION fTycho2GetNearbyObjFromBoxCelSimple(TEXT,FLOAT8,FLOAT8,FLOAT8,FLOAT8)
...
CREATE FUNCTION fTycho2GetNearbyObjFromBoxCel(coo TEXT, lon_c FLOAT8, lat_c FLOAT8,
  ...
  RETURNS SETOF tObjCel AS $$
    DECLARE
      ...
      ndiv INT4;
    BEGIN
      tbl := 'Tycho2';  -- constant: table name
...
-- private
CREATE FUNCTION _fTycho2GetObjFromRectEq(tInternalRectArgs)
  RETURNS SETOF tObjCel
  AS 'SELECT objid,ra,dec FROM Tycho2
...
CREATE FUNCTION fTycho2GetObjFromRectEq(FLOAT8,FLOAT8,FLOAT8,FLOAT8)
  RETURNS SETOF tObjCel
  AS 'SELECT _fTycho2GetObjFromRectEq(_fMakeInternalRectArgs($1,$2,$3,$4))'
  LANGUAGE 'sql';
...
CREATE FUNCTION fTycho2mGetNearestObjIDEq(FLOAT8,FLOAT8,FLOAT8)
  RETURNS INT4
  AS 'SELECT o.objid FROM
      (SELECT objid, fDistanceArcMinEq($1,$2,ra,dec) as distance
       FROM _Tycho2m
...
CREATE FUNCTION fTycho2mGetNearbyObjEq(FLOAT8,FLOAT8,FLOAT8)
  RETURNS SETOF tObjCelAndDistance
  AS 'SELECT o.*
      FROM (
       SELECT objid,ra,dec,
...
CREATE FUNCTION fTycho2mGetNearbyObjCel(TEXT, FLOAT8, FLOAT8, FLOAT8)
  RETURNS SETOF tObjCelAndDistance
  AS 'SELECT o.*
      FROM (
       SELECT objid,
...
CREATE FUNCTION fTycho2mGetNearbyObjFromBoxCel(coo TEXT, lon_c FLOAT8, lat_c FLOAT8,
                                              lon_r FLOAT8, lat_r FLOAT8 )
  RETURNS SETOF tObjCel AS $$
    DECLARE
      tbl TEXT;
      ...
      ndiv INT4;
    BEGIN
      tbl := '_Tycho2m';  -- constant: table name
      -- followings should be tuned for performance
      min_ndiv := 2;    -- constant: minimum num of division
      max_ndiv := 90;   -- constant: maximum num of division
      i := 0;
...
-- private
CREATE FUNCTION _fTycho2mGetObjFromRectEq(tInternalRectArgs)
  RETURNS SETOF tObjCel
  AS 'SELECT objid,ra,dec FROM _Tycho2m
      WHERE (($1.lon1 <= ra AND ra <= $1.lon2) OR
             ($1.lon3 <= ra AND ra <= $1.lon4)) AND
            (($1.lat1 <= dec AND dec <= $1.lat2) OR
             ($1.lat3 <= dec AND dec <= $1.lat4))'
...
CREATE FUNCTION fTycho2mGetObjFromRectEq(FLOAT8,FLOAT8,FLOAT8,FLOAT8)
  RETURNS SETOF tObjCel
  AS 'SELECT _fTycho2mGetObjFromRectEq(_fMakeInternalRectArgs($1,$2,$3,$4))'
  LANGUAGE 'sql';
