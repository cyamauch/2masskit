...
CREATE FUNCTION fTwomassGetNearestObjIDEq(arg1 FLOAT8, arg2 FLOAT8, arg3 FLOAT8)
  ...
  AS $$
    DECLARE
      rt INT4;
    BEGIN
      EXECUTE _fTwomassGetSqlForRadialSearch(arg1, arg2, arg3, TRUE) INTO rt;
      RETURN rt;
    END
...
CREATE FUNCTION fTwomassGetNearbyObjEq(arg1 FLOAT8, arg2 FLOAT8, arg3 FLOAT8)
  ...
  AS $$
    BEGIN
      RETURN QUERY EXECUTE _fTwomassGetSqlForRadialSearch(arg1, arg2, arg3, FALSE);
      RETURN;
    END
  $$ LANGUAGE 'plpgsql';
...
CREATE FUNCTION fTwomassGetNearbyObjFromBoxCelSimple(arg1 TEXT, 
  ...
  AS $$
    BEGIN
      RETURN QUERY EXECUTE _fTwomassGetSqlForBoxSearchSimple(arg1, arg2, arg3, arg4, arg5);
      RETURN;
    END
  $$ LANGUAGE 'plpgsql';
  ...
  RETURNS SETOF tObjCel AS $$
    ...
    BEGIN
      ...
      IF ( CAST(min_ndiv AS FLOAT8) <= lon_r/lat_r ) THEN
        ...
        END IF;
        WHILE ( i < ndiv ) LOOP
          RETURN QUERY EXECUTE _fTwomassGetSqlForBoxSearchDiv(coo,lon_c,lon_r,i,ndiv,lat_c,lat_r,0,1);
          i := i + 1;
        END LOOP;
      ELSIF ( CAST(min_ndiv AS FLOAT8) <= lat_r/lon_r ) THEN
        ...
        END IF;
        WHILE ( i < ndiv ) LOOP
          RETURN QUERY EXECUTE _fTwomassGetSqlForBoxSearchDiv(coo,lon_c,lon_r,0,1,lat_c,lat_r,i,ndiv);
          i := i + 1;
        END LOOP;
      ELSE
        RETURN QUERY EXECUTE _fTwomassGetSqlForBoxSearchDiv(coo,lon_c,lon_r,0,1,lat_c,lat_r,0,1);
      END IF;
      RETURN;
    END
  $$ LANGUAGE 'plpgsql';
...
CREATE FUNCTION fTwomassGetObjFromRectEq(ra1 FLOAT8, ra2 FLOAT8, dec1 FLOAT8, dec2 FLOAT8)
  RETURNS SETOF tObjCel
  AS $$
    DECLARE
    ...
    BEGIN
      ...
      RETURN QUERY EXECUTE
        _fTwomassGetSqlForRectangularSearch(a1.lon1,a1.lon2,a1.lon3,a1.lon4,a1.lat1,a1.lat2);
      IF -90.0 <= a1.lat3 AND -90.0 <= a1.lat4 THEN
        RETURN QUERY EXECUTE
          _fTwomassGetSqlForRectangularSearch(a1.lon1,a1.lon2,a1.lon3,a1.lon4,a1.lat3,a1.lat4);
      END IF;
      RETURN;
    END
  $$ LANGUAGE 'plpgsql';
