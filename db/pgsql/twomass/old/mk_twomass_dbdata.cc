...
/* Time-stamp: <2011-10-17 12:02:06 cyamauch> */
...
int main( int argc, char *argv[] )
{
    ...
    if ( fh_bin.openf("w", "_tmp_%03d.bin",bin_ix) < 0 ) {
	...
	while ( (line=cat_in.getline()) != NULL ) {
	    ...
	    ipos_table_ptr[j].cxi = cxi;
	    ipos_table_ptr[j].cyi = cyi;
	    ipos_table_ptr[j].czi = czi;
	...
	/* Output temporary binary files */
	deci_begin = ipos_table_ptr[0].deci;
	deci_end   = ipos_table_ptr[0].deci;
	for ( k=0 ; k < j ; k++ ) {
	    if ( deci_begin[k].deci < dec_begin ) 
		dec_begin = deci_begin[k].deci;
	    if ( deci_end < deci_begin[k].deci ) 
		deci_end = deci_begin[k].deci;
	    fh_bin.write(ipos_table_ptr, sizeof(*ipos_table_ptr) * j);
	}
	n_entries[bin_ix] += j;
	n_all_entries += j;
	/* */
	sio.eprintf("[INFO] i=%zd: DEC range: %.15g - %.15g\n", i,
		    DECI2DEC(deci_begin), DECI2DEC(deci_end));
	if ( deci_begin < max_deci ) {
	    sio.eprintf("[WARNING] Overwrapped DEC in cat file: %s; "
			"concatenating temp binary file ...\n",
			filename.cstr());
	    /* to be continued ... */
	}
	else {
	    fh_bin.close();
	    if ( fh_bin.openf("w", "_tmp_%03d.bin",bin_ix) < 0 ) {
		sio.eprintf("[ERROR] fh_bin.openf(\"w\",...) failed\n");
		goto quit;
	    }
	    bin_ix ++;
	}
    ...
    sio.eprintf("[INFO] number of all rows: %lld\n", n_all_entries);

    /*
     * Output data files for child tables
     */
	...
	sio.printf("Reading temp file: %03zd ...\r", i);
	sio.flush();

	ipos_table.resize(n_entries[i]);
	if ( fh_bin.openf("r", "_tmp_%03zd.bin",i) < 0 ) {
	    sio.eprintf("[ERROR] fh_bin.openf(\"r\",...) failed\n");
	    goto quit;
	}
	fh_bin.read(ipos_table_ptr, sizeof(*ipos_table) * n_entries[i]);
	fh_bin.close();

	/* Sort ipos_entry */
	qsort(ipos_table_ptr, n_entries[i], sizeof(*ipos_table_ptr), 
	      &cmp_xyzi_tbl);
	max_czi = ipos_table_ptr[n_entries[i] - 1].czi;
